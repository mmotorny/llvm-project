...
TEST_F(SymbolLocatorTest, TheBatchSearchesConcurrently) {
  // Every task has to be inside the locator before any of them is let go, which
  // a serial batch could never manage. No more tasks than the pool can run at
  // once, or the ones that never started would hold the others there.
